...
def show_dashboard():
    ...
    # ---------------- DROPDOWNS ----------------
    country_dropdown = Combobox(tab_summary, textvariable=selected_country, values=country_list, state="readonly")
    country_dropdown.pack(padx=20, pady=5, anchor="nw")
    year_dropdown = Combobox(tab_summary, textvariable=selected_year, values=all_years, state="readonly")
    year_dropdown.pack(padx=20, pady=5, anchor="nw")

    # Label for stats
    country_stats_label = tk.Label(tab_summary, font=("Consolas", 12), justify="left")
    ...
    def update_country_trend():
        ...
        years, gdp_values = extract_country_trend(country_data, start_year=1960, end_year=2024)

        fig = plt.Figure(figsize=(10, 4))
        ax = fig.add_subplot(111)
        ax.plot(years, gdp_values, marker='o', color='blue')
    ...
    def update_region_chart():
        ...
        if region_mode.get() == "bar":
            ...
            fig = plot_region_gdp(region_filtered, {"year": selected_year.get(), "region": continent})
        else:
            fig = plot_continent_gdp_pie(data, {"year": selected_year.get()})
    ...
    def update_year_chart():
        ...
        filtered_for_year = data
        if year_mode.get() == "line":
            fig = plot_year_gdp_line(filtered_for_year, {"year": selected_year.get()})
        else:
            fig = plot_year_gdp_pie(filtered_for_year, selected_year.get())

        year_canvas = FigureCanvasTkAgg(fig, master=year_frame)
        year_canvas.draw()
        year_canvas.get_tk_widget().pack(fill="both", expand=True)

    ttk.Button(
        tab_year,
        text="Toggle Line / Pie",
        command=lambda: (
    ...
    def update_country_stats(event=None):
        ...
        country_data = [row for row in data if row[country_key] == country_name]
        country_gdp_value = [float(row[year]) for row in country_data if row.get(year) not in ("", None)]
        country_sum = sum(country_gdp_value)
        country_avg = country_sum / len(country_gdp_value) if country_gdp_value else 0

        continent = next(row.get("Continent") for row in data if row[country_key] == country_name)
        region_data = [row for row in data if row.get("Continent") == continent]
        region_gdp_values = [float(row[year]) for row in region_data if row.get(year) not in ("", None)]
        region_avg = sum(region_gdp_values) / len(region_gdp_values) if region_gdp_values else 0
